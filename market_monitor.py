...
class MarketMonitor:
    ...
    def _has_latest_data(self, local_df, expected_date):
        ...
        if local_df.empty:
            logger.debug("本地数据为空，无法检查期望日期 %s", expected_date)
            return False
        
        # 🔧 修复：精确检查期望日期是否存在
        expected_date_records = local_df[local_df['date'].dt.date == expected_date]
        has_expected_date = not expected_date_records.empty
        
        if has_expected_date:
            # 检查期望日期的数据是否有效（净值不为空）
            valid_records = expected_date_records.dropna(subset=['net_value'])
            if not valid_records.empty:
                logger.debug("基金本地数据包含 %s 的 %d 条有效记录，最新净值为: %.4f", 
                           expected_date, len(valid_records), valid_records['net_value'].iloc[-1])
                return True
            else:
                logger.debug("基金本地数据包含 %s 的日期，但净值数据为空", expected_date)
                return False
        else:
            # 🔧 关键修复：明确记录缺少的日期
            available_dates = sorted(local_df['date'].dt.date.unique())
            latest_available = available_dates[-1] if available_dates else None
            logger.debug("基金本地数据缺少期望日期 %s，最新可用日期为: %s (共 %d 个日期)", 
                        expected_date, latest_available, len(available_dates))
            return False
            ...
            try:
                ...
                if not df.empty and 'date' in df.columns and 'net_value' in df.columns:
                    ...
                    if pd.isna(df['date']).any():
                        logger.warning("基金 %s 的日期列解析有问题，尝试重新解析", fund_code)
                        df['date'] = pd.to_datetime(df['date'], errors='coerce')
                        df = df.dropna(subset=['date'])
                    
                    logger.info("本地已存在基金 %s 数据，共 %d 行，最新日期为: %s", 
                              fund_code, len(df), df['date'].max().date() if not df.empty else "无数据")
                    return df
            except Exception as e:
    ...
    def _fetch_fund_data(self, fund_code, latest_local_date=None):
            ...
            try:
                ...
                if latest_local_date:
                    ...
                    if new_df_page.empty:
                        # 如果当前页没有新数据
                        if has_new_data:
                            # 如果之前已经找到过新数据，说明已经获取完所有新数据
                            logger.info("基金 %s 已获取所有新数据（共 %d 行），爬取结束", fund_code, len(pd.concat(all_new_data)))
                            break
                        elif page_index == 1:
                            # 如果是第一页就没新数据，说明没有新数据
                            logger.info("基金 %s 第一页无新数据，爬取结束", fund_code)
                            break
                        else:
                            # 继续爬取下一页，可能有新数据
                            logger.debug("基金 %s 第 %d 页无新数据，继续爬取", fund_code, page_index)
                    else:
                        has_new_data = True
                        all_new_data.append(new_df_page)
                        logger.info("第 %d 页: 发现 %d 行新数据 (日期范围: %s 至 %s)", 
                                  page_index, len(new_df_page), 
                                  new_df_page['date'].min().date(), new_df_page['date'].max().date())
                else:
                    # 如果是首次下载，则获取所有数据
                ...
                # 🔧 修复：检查是否已经追溯到本地数据
                if latest_local_date and not df_page.empty and (df_page['date'].dt.date <= latest_local_date).any():
                    logger.info("基金 %s 已追溯到本地数据 %s，增量爬取结束", fund_code, latest_local_date)
                    break
        ...
        if all_new_data:
            ...
            if not new_combined_df.empty:
                logger.info("基金 %s 成功获取 %d 行新数据 (日期范围: %s 至 %s)", 
                          fund_code, len(new_combined_df),
                          new_combined_df['date'].min().date(), new_combined_df['date'].max().date())
                return new_combined_df[['date', 'net_value']].sort_values('date')
            else:
                logger.warning("基金 %s 合并后新数据为空", fund_code)
        else:
            logger.info("基金 %s 未获取到任何新数据", fund_code)
        return pd.DataFrame()
        ...
        try:
            ...
            if not df.empty and 'date' in df.columns:
                ...
                if 'close' in df.columns:
                    df = df.rename(columns={'close': 'net_value'})
                elif 'net_value' not in df.columns:
                    # 如果没有合适的字段，使用第一个数值列
                    numeric_columns = df.select_dtypes(include=[np.number]).columns
                    if len(numeric_columns) > 0:
                        df = df.rename(columns={numeric_columns[0]: 'net_value'})
                    else:
                        logger.warning("大盘指数文件没有数值列")
                        return pd.DataFrame()
                
                df = df[['date', 'net_value']].dropna(subset=['date', 'net_value'])
                df = df.sort_values(by='date', ascending=True).reset_index(drop=True)
                logger.info("大盘指数 %s 数据加载成功，共 %d 行，最新日期为: %s", 
                          self.index_code, len(df), df['date'].max().date())
                return df
            else:
                logger.warning("大盘指数文件格式不正确，缺少必要列")
                return pd.DataFrame()
        except Exception as e:
    ...
    def _get_latest_signals(self, fund_code, df):
        ...
        try:
            ...
            if processed_df is None:
                ...
                return {
                    'fund_code': fund_code, 'latest_net_value': np.nan, 'rsi': np.nan, 'ma_ratio': np.nan,
            ...
            # 🔧 修复：确保净值是数值类型
            safe_net_value = float(latest_net_value) if not np.isnan(latest_net_value) else np.nan

            return {
                'fund_code': fund_code,
                'latest_net_value': safe_net_value,
                'rsi': latest_rsi,
    ...
    def _safe_float_format(self, value):
        """安全地格式化浮点数，避免转换错误"""
    ...
    def generate_portfolio_recommendation(self):
        ...
        with open(self.portfolio_output_file, 'w', encoding='utf-8') as f:
            ...
            # 🔧 修复：使用安全格式化
            f.write(f"📈 **最新净值**: {self._safe_float_format(self.index_data['latest_net_value'])}\n")
            f.write(f"📊 **RSI**: {self._safe_float_format(self.index_data['rsi'])}\n")
            f.write(f"📉 **MA_Ratio**: {self._safe_float_format(self.index_data['ma_ratio'])}\n")
    ...
    def generate_detailed_report(self):
        ...
        with open(self.output_file, 'w', encoding='utf-8') as f:
            ...
            for code, data in self.fund_data.items():
                # 🔧 修复：安全处理净值显示
                net_value_str = self._safe_float_format(data['latest_net_value'])
                if net_value_str == "N/A":
                    rsi_str = "N/A"
                    ma_ratio_str = "N/A"
                    macd_signal = "N/A"
                else:
                    ...
                    macd_signal = '金叉' if data['macd_diff'] > 0 else '死叉' if data['macd_diff'] < 0 else '无信号'
                
                line = (
                    f"| {data['fund_code']} | {net_value_str} | {rsi_str} | {ma_ratio_str} | "
                    f"{macd_signal} | "
    ...
    def _backtest_strategy(self, fund_code, df):
        ...
        if position == 1:
            ...
            ret = (sell_price - buy_price) / buy_price
            trades.append({'buy_date': df.iloc[i-1]['date'], 'sell_date': df.iloc[-1]['date'], 'return': ret, 'type': 'final_sell'})
        ...
        try:
            ...
            for fund_code in self.fund_codes:
                ...
                if not local_df.empty:
                    ...
                    logger.info("检查基金 %s: 本地最新日期 %s vs 期望日期 %s, 数据量 %d, 包含期望数据: %s", 
                              fund_code, latest_local_date, expected_latest_date, data_points, has_expected_data)
                    ...
                    if data_points >= min_data_points and has_expected_data:
                        ...
                        if not has_expected_data:
                            logger.info("⚠️ 基金 %s 本地数据缺少期望日期 %s 的有效数据 (最新日期为 %s)，需要从网络获取新数据。", 
                                      fund_code, expected_latest_date, latest_local_date)
                        if data_points < min_data_points:
                            logger.info("⚠️ 基金 %s 本地数据量不足（仅 %d 行，需至少 %d 行），需要从网络获取。", 
            ...
            logger.info("需要更新的基金数量: %d / %d", len(fund_codes_to_fetch), len(self.fund_codes))
    ...
    def _process_single_fund(self, fund_code):
        ...
        logger.info("处理基金 %s，本地最新日期: %s", fund_code, latest_local_date)
        ...
        if not new_df.empty:
            ...
            if not local_df.empty:
                ...
                logger.info("基金 %s 合并后数据: %d 行 (本地 %d + 新增 %d)", 
                          fund_code, len(df_final), len(local_df), len(new_df))
            else:
                df_final = new_df
                logger.info("基金 %s 首次下载，获取 %d 行历史数据", fund_code, len(df_final))
            
            self._save_to_local_file(fund_code, df_final)
            # 使用最新的100条数据计算信号
            signal_data = self._get_latest_signals(fund_code, df_final.tail(100))
            logger.info("基金 %s 信号计算完成: %s (净值: %.4f)", 
                      fund_code, signal_data['action_signal'], signal_data['latest_net_value'])
            return signal_data
