...
class MarketMonitor:
        ...
        if fund_codes_to_fetch:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
                ...
                for future in concurrent.futures.as_completed(future_to_code):
                    ...
                    try:
                        df = future.result()
                        result = self._get_latest_signals(fund_code, df)
    ...
    def _backtest_strategy(self, fund_code, df):
        ...
        # 计算回测指标
        if trades and 'return' in trades[-1]:
        ...
        # 考虑到可能没有完成交易，使用完整净值序列来计算回撤和夏普比率
        equity_curve = df['net_value'].cumprod() if 'net_value' in df else None
        if equity_curve is not None and not equity_curve.empty:
            ...
            sharpe_ratio = np.mean(daily_returns) / np.std(daily_returns) * np.sqrt(252) if len(daily_returns) > 1 else np.nan
        else:
            max_drawdown = np.nan
            sharpe_ratio = np.nan
